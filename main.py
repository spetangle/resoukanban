...
WTTR_LOCATION = "Xicheng,Beijing"            

# 4. 屏幕显示文字
# 天气页面左上角的自定义标题，你可以改成 "北京市 | 我的温馨小窝" 等等
CITY_DISPLAY_NAME = "西城区 | 北京市"      
...
def task_weather_dashboard():
    ...
    draw.rounded_rectangle([(235, 45), (385, 130)], radius=8, outline=0, fill=None)
    
    # 🔧修改点 5：调整右侧黑框内文字位置使其居中 (X 从 245 移到 255，Y 轴均匀排开)
    draw.text((255, 56), f"{weather['wind_info']}", font=font_small, fill=0)
    draw.text((255, 80), f"湿度 {weather['humidity']}", font=font_small, fill=0)
    draw.text((255, 104), f"体感 {weather['feel_temp']}", font=font_small, fill=0)
